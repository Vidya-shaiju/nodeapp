...
module.exports = {
    ...
    getCourses: function (req, res) {
        ...
        connection.query('SELECT * FROM courses', function (err, result, fields) {
            ...
            res.status(200).json({ code: "courseFound", data: result });
        });
    },
    getCourseById: function (req, res) {
        ...
        connection.query('SELECT * FROM courses where id=?', [taskId], function (err, result, fields) {
            if (err) {
                console.log(err);
                return res.status(500).json({ code: "courseNotFound", message: "error in database while finding course" });
            }
            // checking if no data is returned from database then returning response
            if (result.length === 0) {
                return res.status(404).json({ code: "CourseNotFound", message: "Course not found" });
            }
